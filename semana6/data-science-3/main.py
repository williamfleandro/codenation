...
principalComponents = pca.fit_transform(fifa)
...
print(pca.explained_variance_ratio_[0].round(3))


# In[13]:


len([sum(pca.explained_variance_ratio_[:i+1]) for i in range(len(pca.explained_variance_ratio_)) if sum(pca.explained_variance_ratio_[:i]) <= 0.95])
